...
public class FileUtils {
    ...
    public static double[][] readSvmFileToMatrix(String filename){
        String rawString = readFile(filename);
        String[] lines = rawString.split("\n");
        //纵坐标应该是冒号的个数
        double[][] vectors = new double[lines.length][lines[0].split(":").length-1];
        try {
            for (int i = 0; i < lines.length; ++i) {
                String[] items = lines[i].split(":");
                //第一个字符串不要
                for (int j = 0; j < items.length-1; ++j) {
                    String[] subitems = items[j+1].split("\t");

                    vectors[i][j] = Double.parseDouble(subitems[0]);
                    //System.out.println(vectors[i][j]);
                }
            }
        } catch (Exception e){
            e.printStackTrace();
